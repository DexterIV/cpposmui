#pragma once
// Compact public-domain SHA-256 (used for OAuth2 PKCE S256 challenges).
#include <array>
#include <cstdint>
#include <cstring>
#include <string>
...
inline std::string base64url(const uint8_t* p, size_t n) {
    ...
    for (size_t i = 0; i < n; i += 3) {
        ...
        if (i+1 < n) out.push_back(tbl[(v >> 6) & 0x3f]);
        if (i+2 < n) out.push_back(tbl[v & 0x3f]);
    }
    return out;
}
